...
const IngresoPersonalForm = ({ onSubmit, isSubmitting = false, onCancel }) => {
    ...
    const handleProceedToStep2 = async () => {
        ...
        if (isCedulaValid) {
            ...
            setSubmitError(
                "❌ No se puede continuar. La cédula ya existe en el sistema o hay un error en la validación.",
            );
        }
    };
    ...
    const handleProceedToStep3 = () => {
        ...
        setErrors({});
        toast.success(
            "✅ Datos laborales y personales VALIDADOS, REVISA Y GUARDA",
        );
    };
    ...
    const getMissingStep2Fields = () => {
        ...
        if (!formData.fecha_ingreso)
            missingFields.push(fieldLabels.fecha_ingreso);
        if (!formData.contrato) missingFields.push(fieldLabels.contrato);
        if (!formData.jefe_inmediato)
            missingFields.push(fieldLabels.jefe_inmediato);
        if (!formData.fecha_nacimiento)
            missingFields.push(fieldLabels.fecha_nacimiento);
        if (!formData.genero) missingFields.push(fieldLabels.genero);
        if (!formData.ciudad) missingFields.push(fieldLabels.ciudad);
        if (formData.ciudad === "BOGOTA" && !formData.localidad)
            missingFields.push(fieldLabels.localidad);
        if (!formData.lugar) missingFields.push(fieldLabels.lugar);
        if (!formData.direccion_residencia)
            missingFields.push(fieldLabels.direccion_residencia);
        if (!formData.eps) missingFields.push(fieldLabels.eps);
        if (!formData.fondo_pensiones)
            missingFields.push(fieldLabels.fondo_pensiones);
        if (!formData.arl) missingFields.push(fieldLabels.arl);
        if (!formData.cantidad_hijos)
            missingFields.push(fieldLabels.cantidad_hijos);
        if (!formData.contacto_emergencia_nombre)
            missingFields.push(fieldLabels.contacto_emergencia_nombre);
        if (!formData.contacto_emergencia_telefono)
            missingFields.push(fieldLabels.contacto_emergencia_telefono);
        if (formData.contrato === "TEMPORAL" && !formData.nombre_temporal)
            missingFields.push(fieldLabels.nombre_temporal);
        ...
        <div className="space-y-4">
            <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded">
                ...
                <p className="text-sm text-green-700 mt-1">
                    Información del puesto y área de trabajo
                </p>
            </div>
                                ...
                                {formData.tipo_contrato_laboral === "FIJO" && (
                                    <div>
                                        <FormField
                                            ...
                                            type="date"
                                            value={
                                                formData.fecha_terminacion_contrato
                                            }
                                            onChange={handleChange}
                                            onBlur={handleBlur}
                                            min={getTodayDate()}
            ...
            <div className="bg-blue-50 border-l-4 border-blue-500 p-3 rounded text-sm text-blue-800 mt-6">
                ...
                <p>
                    Información personal, beneficiarios y datos de seguridad
                    social
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div className="w-full px-4 py-2 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-900 font-semibold">
                        {formData.estado
                            ? formatEstado(formData.estado)
                            : "Se determina según el tipo de contrato"}
                    </div>
                    <p className="text-xs text-gray-500 mt-1">
                        {formData.contrato
                            ? `Asignado automáticamente para contrato: ${formData.contrato}`
    ...
    const renderStep3 = () => {
        ...
        const formatValue = (field, value) => {
            ...
            const dropdownMaps = {
                ...
                estado: {
                    PENDIENTE_APROBACION_JURIDICO:
                        "Pendiente Aprobación Jurídica",
                    EN_PROCESO_DE_CONTRATACION: "En Proceso de Contratación",
                    ACTIVO: "Activo",
                    RETIRADO: "Retirado",
                    PENDIENTE_RETIRO_JURIDICO: "Pendiente Retiro Jurídico",
            ...
            <div className="space-y-6">
                <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h4 className="font-semibold text-green-900">
                                Resumen - Verifica tus datos
                            </h4>
                            <p className="text-sm text-green-700 mt-0.5">
                                Revisa toda la información antes de crear el
                                empleado
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-gray-50 p-3 rounded">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("cedula", formData.cedula)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("nombre", formData.nombre)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("celular", formData.celular)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-gray-50 p-3 rounded">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("cargo", formData.cargo)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("area", formData.area)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                        ...
                        <div className="bg-gray-50 p-3 rounded">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("contrato", formData.contrato)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-gray-50 p-3 rounded">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("genero", formData.genero)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("estado", formData.estado)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                        ...
                        <div className="bg-gray-50 p-3 rounded">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("ciudad", formData.ciudad)}
                            </p>
                        </div>
                        {formData.ciudad === "BOGOTA" && (
                            <div className="bg-gray-50 p-3 rounded">
                        ...
                        <div className="bg-gray-50 p-3 rounded">
                            ...
                            <p className="text-sm text-gray-800 mt-1">
                                {formatValue("lugar", formData.lugar)}
                            </p>
                        </div>
                        <div className="bg-gray-50 p-3 rounded">
                            <p className="text-xs text-gray-500 font-semibold uppercase">
                ...
                <div className="bg-blue-50 border-l-4 border-blue-500 p-3 rounded text-sm text-blue-800">
                    ...
                    <p>
                        Todos los datos han sido validados. Puedes confirmar
                        para crear el empleado o editar cualquier sección.
                    </p>
                </div>
            </div>
        );
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center gap-2 mb-6">
                <div
                    className={`flex items-center justify-center w-10 h-10 rounded-full font-semibold transition-all ${step >= 1 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-600"}`}
                >
                    1
                </div>
                <div
                    className={`flex-1 h-1 rounded transition-all ${step > 1 ? "bg-blue-600" : "bg-gray-200"}`}
                />
                <div
                    className={`flex items-center justify-center w-10 h-10 rounded-full font-semibold transition-all ${step >= 2 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-600"}`}
                >
                    2
                </div>
                <div
                    className={`flex-1 h-1 rounded transition-all ${step > 2 ? "bg-blue-600" : "bg-gray-200"}`}
                />
                <div
                    className={`flex items-center justify-center w-10 h-10 rounded-full font-semibold transition-all ${step >= 3 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-600"}`}
                >
                    3
                </div>
            </div>
